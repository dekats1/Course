...
public class ProfileService {
    ...
    public static SellerRequest changePassword(String userName, String oldPassword, String newPassword, String ownerName) {
            ...
            if (resultSet.next()) {
                ...
                if (PasswordUtil.checkPassword(oldPassword, passwordHash, salt)) {
                    ...
                    try (PreparedStatement preparedStatement2 = connection.prepareStatement(sql2)) {
                        ...
                        if (affectedRows == 0) {
                            return new SellerRequest(false, "Не удалось обновить пароль");
                        }
                        LogService.addLog(ownerName, "Был изменен пароль");

                        return new SellerRequest(true, "Пароль успешно изменён");
                    }
                } else {
                    return new SellerRequest(false, "Неверный текущий пароль");
                }
            } else {
                return new SellerRequest(false, "Пользователь не найден");
            }
        } catch (SQLException e) {
            return new SellerRequest(false, "Ошибка базы данных: " + e.getMessage());
        }
    }

    public static SellerRequest userData(String userName) {
        ...
        try (Connection connection = DataBaseConnection.getConnection();
            ...
            if (resultSet.next()) {
                ...
                return new SellerRequest(firstName+ " " + lastName, dateAt);
            }
